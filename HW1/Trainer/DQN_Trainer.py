    ...
    def train(self):
        for epoch in range(self.Epoch):
            ...
            for i in range(self.train_Data.shape[0]-1):
                ...
                daily_reward, terminate = self.Env.execute_action(action, four_features[0], tomorrow_open_prize) # put current action and open_prize in int 
                
                if i % self.verbose_round ==0 :
                    print("Day:%s, Status:%s, income:%s, Choosen-Action:%s, reward:%s, ter : %s "%(i, status, income, action, daily_reward, terminate))
                if terminate:
                    print("Violate rule in day:", i)
                    self.Agent.memory.append(daily_memory)
                    break
                    daily_memory = (daily_reward, action_var)
                self.Agent.memory.append(daily_memory)
            # print(self.Agent.reward_pool)
            print("Day:%s, Status:%s, Income:%s,  Action:%s, reward:%s, ter : %s "%(i, status, income, action, daily_reward, terminate))
